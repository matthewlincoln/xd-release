...
#SBATCH -J t1d.ibd
#SBATCH --array=1-20
#SBATCH -o ${t1d_direc}/7_ibd/2_parallel_outputs/scripts/t1d.run.genome.%%a.out
#SBATCH -e ${t1d_direc}/7_ibd/2_parallel_outputs/scripts/t1d.run.genome.%%a.err

plink --bfile ${t1d_direc}/7_ibd/1_ld_pruned/t1d.ld.pruned \\
    --genome full \\
    --min 0.185 \\
    --parallel \$SLURM_ARRAY_TASK_ID 20 \\
    --out ${t1d_direc}/7_ibd/2_parallel_outputs/outputs/t1d.ibd.\${SLURM_ARRAY_TASK_ID}" > \
    ${t1d_direc}/7_ibd/2_parallel_outputs/scripts/t1d.run.genome.sh

jobid=$(sbatch --parsable ${t1d_direc}/7_ibd/2_parallel_outputs/scripts/t1d.run.genome.sh)
