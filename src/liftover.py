...
### Noah Connally
### 2017/06/01
### This script takes our Immunochip data for CeD, IBD, MS, SLE G, SLE O, and T1D and converts it
#   from build HG18 to HG19. This enables us to use 1000 Genomes Phase III in the analysis, as these
#   data are HG19.
### This code was modified from that given on:
#   http://genome.sph.umich.edu/wiki/LiftMap.py

import sys, os

def usage():
    print("%s -m old.map -p old.ped -d old.dat -o new.prefix -c chain.file -b liftover.binary" % sys.argv[0] )
...
LIFTED_SET = set()
UNLIFTED_SET = set()
def liftBed(fin, fout, funlifted, lift, chain):
    params = dict()
    params['LIFTOVER_BIN'] = lift
    params['OLD'] = fin
    params['CHAIN'] = chain
    params['NEW'] = fout
    params['UNLIFTED'] = fout + '.unlifted'
    # for key in params:
    #     params[key] = params[key].replace(' ', '\ ').replace('(', '\(').replace(')', '\)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', dest='liftoverBin', required = True)
    args = parser.parse_args()

    oldBed = (args.mapFile + '.bed').replace('"', '')
    makesure(map2bed(args.mapFile, oldBed),
             'map->bed succ')

    newBed = (args.prefix + '.bed').replace('"', '')
    unlifted = (args.prefix + '.unlifted').replace('"', '')
    makesure(liftBed(oldBed, newBed, unlifted, args.liftoverBin, args.chainFile),
             'liftBed succ')

    newMap = (args.prefix + '.map').replace('"', '')
